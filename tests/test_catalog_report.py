...
def _build_recon_fixture(tmp_path: Path) -> Path:
    ...
    sectors_dir.joinpath("T00_S00.bin").write_bytes(
        b"DATA=HELLO.TXT////" + pointer_bytes
    )
...
def test_catalog_report_defaults_to_recon_dir_output(tmp_path: Path) -> None:
    recon_dir = _build_recon_fixture(tmp_path)
    ...
    json_path = recon_dir / "catalog_report" / "catalog_report.json"
    txt_path = recon_dir / "catalog_report" / "catalog_report.txt"
